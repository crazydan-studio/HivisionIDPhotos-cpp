...
void human_mating (Hivision_config hivision){
	...
	std::string OutPathStr(hivision.out_image_path);
    ...
    cv::Mat bgra_img=human_matting(ModelPath,image,hivision.thread_num);
    cv::imwrite(OutPathStr+"/human_matting."+hivision.out_image_type, bgra_img);
...
int ID_photo (Hivision_config hivision,int out_size_kb,bool layout_phot){
	...
	cv::Vec3b newBackgroundColor(hivision.background_color.b,hivision.background_color.g,hivision.background_color.r);
	...
	free(face_detector);
    cv::Mat hd_result = photo_adjust(human_matting_params, add_background_img, hivision.out_image_height,hivision.out_image_width,hivision.background_color.r,hivision.background_color.g,hivision.background_color.b, hivision.head_measure_ratio);
	...
	if(out_size_kb>0){
		...
		resizeImageToKB(hd_result,OutPathStr+"/result_kb."+hivision.out_image_type,out_size_kb);
	}
    ...
    if(layout_phot){
		...
		cv::imwrite( OutPathStr+"/layout_photo."+hivision.out_image_type, result_layout_image);
	 }
	cv::imwrite( OutPathStr+"/result_hd."+hivision.out_image_type, hd_result);
	cv::imwrite(OutPathStr+"/result_standard."+hivision.out_image_type, standard_result);
	return 1;
